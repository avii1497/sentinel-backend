...
class Database {
    ...
    public function __construct() {
        ...
        // ✅ Use ONE consistent options array
        $options = [
            PDO::ATTR_ERRMODE            => PDO::ERRMODE_EXCEPTION,
            PDO::ATTR_DEFAULT_FETCH_MODE => PDO::FETCH_ASSOC,
            PDO::ATTR_EMULATE_PREPARES   => false, // ✅ critical for consistent prod behavior
        ];
    ...
    public function getUserByEmail(string $email): ?array {
        ...
        $stmt->execute(['email' => $email]); // ✅ consistent named param key
    ...
    public function registerUser(array $user, array $roleData = []): int {
        ...
        try {
            $sql = "INSERT INTO users
                    ...
                    VALUES
                    (:first_name, :last_name, :email, :password_hash, :role, 0, 0, NOW())";
            ...
            switch ($user['role']) {
                case 'owner':
                    $this->createOwner($userId, $roleData);
                    break;
                case 'agent':
                    $this->createAgent($userId, $roleData);
                    break;
                case 'customer':
                case 'premium_customer':
                    $this->createCustomer($userId, $roleData);
                    break;
            }
